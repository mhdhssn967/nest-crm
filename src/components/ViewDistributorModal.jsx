...
// ✅ FIX: Field is defined at MODULE level (outside ViewDistributorModal).
// When it was inside, React recreated it as a new component type on every
// render, causing inputs to unmount/remount and lose focus after each keystroke.
// Now it has a stable identity across renders, so focus is preserved.
const Field = ({ label, value, editEl, editing }) => (
  <div className="mb-3">
    <div style={{ fontSize: "0.72rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "#888", marginBottom: 2 }}>
      {label}
    </div>
    {editing
      ? editEl
      : <div style={{ fontWeight: 500, color: "#222" }}>{value || <span style={{ color: "#bbb" }}>—</span>}</div>
    }
  </div>
);
    ...
    <Modal show onHide={onClose} size="xl" backdrop="static" scrollable>
      ...
      <Modal.Body style={{ padding: "1.5rem" }}>
        ...
        <div className="row">
          ...
          <div className="col-md-4">
            <Field editing={editing} label="Exclusive" value={form.exclusive}
          ...
          <div className="col-md-4">
            <Field editing={editing} label="Team Size" value={form.teamSize}
              editEl={<Form.Control size="sm" type="number" value={form.teamSize} onChange={f("teamSize")} />} />
          </div>
          <div className="col-md-4">
            <Field editing={editing} label="Year Established" value={form.establishedYear}
          ...
          <div className="col-md-6">
            <Field editing={editing} label="Product Lines Handled" value={form.productLinesHandled}
              editEl={<Form.Control size="sm" value={form.productLinesHandled} onChange={f("productLinesHandled")} />} />
          </div>
          <div className="col-md-6">
            <Field editing={editing} label="Territory Description" value={form.territoryDescription}
          ...
          <div className="col-md-3">
            <Field editing={editing} label="Current Status" value={form.currentStatus}
              editEl={<Form.Select size="sm" value={form.currentStatus} onChange={f("currentStatus")}>
                {["Haven't yet contacted","Called, no response","Contacted and discussed via phone","Online demo done","Live demo done","Hospital presentation done","Agreement Sent & awaiting response","Agreement Signed","Purchased Demo Piece","Doing Sales","Inactive","Terminated"].map(s => <option key={s}>{s}</option>)}
              </Form.Select>} />
          </div>
          <div className="col-md-3">
            <Field editing={editing} label="Last Meeting Date" value={form.lastMeetingDate}
              editEl={<Form.Control size="sm" type="date" value={form.lastMeetingDate} onChange={f("lastMeetingDate")} />} />
          </div>
          <div className="col-md-3">
            <Field editing={editing} label="Next Follow-Up" value={form.nextFollowUp}
              editEl={<Form.Control size="sm" type="date" value={form.nextFollowUp} onChange={f("nextFollowUp")} />} />
          </div>
          <div className="col-md-3">
            <Field editing={editing} label="Added By" value={form.addedByName} editEl={<Form.Control size="sm" value={form.addedByName} disabled />} />
          </div>
          <div className="col-12">
            <Field editing={editing} label="Remarks" value={form.remarks}
